...
const { DiskBucket } = require("../dist/buckets/disk-bucket");
const fs = require('fs')
...
(async function main() {
  const options = {
    ...
    bucket: new DiskBucket({
      filename: './memory.json',
      reader: util.promisify(fs.readFile),
      writer: util.promisify(fs.writeFile),
    }),
  };
  ...
  console.log(
    `Rate Limiter - ${ATTEMPTS} allowed every ${BUCKET_CLEAR_TIME / 1000}s`
  );
  ...
  setInterval(async () => {
    ...
    setTimeout(() => {
      process.stdout.write(
        isAllowed ? colorize(92, " Allowed") : colorize(91, " Blocked")
      );
